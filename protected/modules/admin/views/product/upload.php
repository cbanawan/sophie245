...
<?php 
	$form = $this->beginWidget(
		...
		echo CHtml::hiddenField('catalogId', $catalog->id);
...
function send(){
    ...
    $.ajax({
        url: '<?php echo Yii::app()->createUrl("/admin/product/upload", array("catId" => $catalog->id)); ?>',
        type: 'POST',
        data: formData,
        datatype:'json',
        // async: false,
        ...
        success: function (data) {
			...
			$("#upload-result").append(data.summary.success.insert + ' record(s) inserted...<br />');
			$("#upload-result").append(data.summary.success.updated + ' record(s) updated...<br />');
			$("#upload-result").append(data.summary.failure + ' record(s) failed...<br />');
        },
