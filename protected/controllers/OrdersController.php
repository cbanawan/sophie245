...
class OrdersController extends Controller
{
	 ...
	 */
	public function actionCreate($memberId = null)
	{
		$model=new Orders;
	...
	{
		$products = array_values(CHtml::listData(Products::model()->findAll(), 'id', 'codename'));
		$order = Orders::model()->with('member', 'user', 'orderStatus', 'orderdetails','payments')->findByPk($id);
		
		$this->render('detail',array(
			'order'=>$order,
			'products'=>$products,
			'activeNavItem' => isset($_REQUEST['navItem']) ? $_REQUEST['navItem'] : 'order_details',
		));		
	}
	...
	public function actionIndex()
	{
		...
		$orders = Orders::model()->with('member', 'user', 'orderStatus')
								->findAll();
